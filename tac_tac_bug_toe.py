...
def is_win(player):
    ...
    # bug2 : row win doesnt work. If statement for diagonal not in the good place, adjustment of return statements
    for i in range(3):
        if all([cell == player for cell in board[i]]):  # Rows
            return True
        if all([board[j][i] == player for j in range(3)]):  # Columns
            return True
        # bug3 : diagonal wins not working. fix diagonal conditions
        if board[0][0] == board[1][1] == board[2][2] == player or board[0][2] == board[1][1] == board[2][0] == player:  # Diagonals
            return True
    return False
...
def main():
    ...
    while moves < 9:
        ...
        # bug1 : check for out of range input and raise an exception
